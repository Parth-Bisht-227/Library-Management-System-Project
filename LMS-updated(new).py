...
def add_record():
    ...
    if surety:
        try:
            # bk_id_str = bk_id.get()  # Retrieve value from StringVar
            cursor.execute(
                'INSERT INTO Library (BK_NAME, BK_ID, AUTHOR_NAME, BK_STATUS, CARD_ID) VALUES (?, ?, ?, ?, ?)',
                (bk_name.get(),f'"{bk_id.get()}"', author_name.get(), bk_status.get(), card_id.get()))
            connector.commit()
    ...
    def update():
        ...
        try:
            cursor.execute('UPDATE Library SET BK_NAME=?, BK_STATUS=?, AUTHOR_NAME=?, CARD_ID=? WHERE BK_ID=?',
                           (bk_name.get(), bk_status.get(), author_name.get(), card_id.get(), book_id))
            connector.commit()

            clear_and_display()

            mb.showinfo('Update successful', 'The record was successfully updated.')
...
def change_availability():
    ...
    if BK_status == 'Issued':
        ...
        if issuer is None:
            return
        cursor.execute('UPDATE Library SET bk_status=?, card_id=? WHERE bk_id=?', ('Issued', issuer, BK_id))
        connector.commit()
...
# Variables
lf_bg = 'GreenYellow'  # Left Frame Background Color
rtf_bg = 'GreenYellow'  # Right Top Frame Background Color
rbf_bg = 'GreenYellow'  # Right Bottom Frame Background Color
btn_hlb_bg = 'Green'  # Background color for Head Labels and Buttons
...
root = Tk()
root.title('IT-106 Library Management System Project')
...
# Initialize StringVar for bk_id
bk_id = StringVar()
